...
void jogpad_check() 
{
	...
	dest_step_rate = 0x10;     // constant speed / ADC disabled
	...
	// repeat until button/joystick released
	for(;;) {

		// Get limit pin state
		limit_state = limits_get_state();  //  LIMIT_MASK & (LIMIT_PIN ^ settings.invert_mask);
		jog_bits = get_jog_bits();         // active low, neu abfragen weil in der Schleife
