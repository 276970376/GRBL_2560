...
void jog_waitmsg() {
//  Falls w�hrend Tastendruck von Spindel oder Zero eine Status-Anfrage kommt
...
//  gc_state.coord_offset[N_AXIS];  // Retains the G92 coordinate offset (work coordinates) relative to
  ...
  TIMSK1 &= ~(1<<OCIE1A);  // Timer 1 zun�chst abgeschaltet
  ...
  skip_count = 0;
  skip_max = 8;			// Prescaler nicht �ndern, deshalb so
  ...
  for(;;) { // repeat until button/joystick released  
    ...
    // Maximaler Wert f�r 16-Bit-Timer darf nicht �berschritten werden
    if (step_delay > 65535) {
    		step_delay = 65535;
    	}
...
//  report_realtime_status()  ben�tigt viel Zeit, deshalb Minimalmeldung
    if (sys.rt_exec_state & EXEC_STATUS_REPORT) {
       // status report requested, print short msg only
      printPgmString(PSTR("<Jog"));
      serial_write(88 + jog_select); // 88 = X + 1 = Y etc.
      serial_write(44);
      // Relativposition zum Werkst�ck-Nullpunkt ermitteln aud ausgeben
    ...
    delay_us(100);	// wg. AD-Wandlung ohnehin ben�tigt
...
// Interupt setzt eigenen Counterwert neu und berechnet gew�nschte Steprate.
void jog_isr() {
  if (skip_count == 0) {
    ...
    if (jog_bits == jog_bits_old) { // nothing changed
      if (step_rate < (dest_step_rate - 5)) { // Hysterese f�r A/D-Wandlung
  	...
  	// Delay f�r Step-Impuls im Interrupt, nicht so sehr sch�n, aber einfach
  	delay_us(settings.pulse_microseconds); 
